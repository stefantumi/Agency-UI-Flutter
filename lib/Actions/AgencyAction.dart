...
Future<Agency> createAgency(Agency newAgency) async {
  Uri url = Uri.parse("https://localhost:7210/api/agency");
...
Future<List<Agency>> getAgencyList() async {
  var response = await http.get(Uri.parse("https://localhost:7210/api/agency"));
...
Future<Agency> getAgencyById(int id) async {
  var response = await http.get(Uri.parse("https://localhost:7210/api/agency/$id"));
  if(response.statusCode == 200 || response.statusCode == 201){
    return Agency.fromJson(jsonDecode(response.body));
  }else{
...
Future<http.BaseResponse> updateAgencyById(int id, String newName) async {
  var url = Uri.parse("https://localhost:7210/api/agency/$id");
...
Future<http.BaseResponse> deleteAgencyById(int id) async {
  var response = await http.delete(Uri.parse("https://localhost:7210/api/agency/$id"));
